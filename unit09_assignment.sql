...
SELECT first_name, last_name, SUM(amount) AS 'Total Amount Sold'
FROM staff
JOIN payment
USING (staff_id)          -- ON (staff.staff_id = payment.staff_id)
...
USING (film_id)          -- ON (film.film_id = film_actor.film_id)
GROUP BY title;
...
FROM film
JOIN inventory
USING (film_id)          -- ON (film.film_id = inventory.film_id)
...
FROM customer
JOIN payment
USING (customer_id)          -- ON (customer.customer_id = payment.customer_id)
GROUP BY last_name
...
SELECT address, district, SUM(amount) AS 'Total Revenue'
FROM payment
INNER JOIN rental
USING (rental_id)
  INNER JOIN inventory
...
CREATE VIEW Top_5_Genres_by_GR AS
SELECT name, SUM(amount) AS 'Gross Revenue'
FROM category
  INNER JOIN film_category 
  USING (category_id)
...
SELECT * FROM Top_5_Genres_by_GR;

-- 8c. You find that you no longer need the view top_five_genres. Write a query to delete it.
DROP VIEW Top_5_Genres_by_GR;
